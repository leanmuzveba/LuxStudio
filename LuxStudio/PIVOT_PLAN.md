...
### Phase 12 — Build: Share (retires social + batch export) — DONE
New `lib/screens/share_screen.dart` matching `ui_kit/share/index.html`: static preview frame (no real video player wired here, matching the mockup's own static preview), platform picker (Reels/TikTok/Shorts/Story — purely cosmetic, no per-platform export variant exists), one combined caption blob (`SocialCopy.displayBlob`, new getter) with copy/regenerate, Apply Branding toggle (reuses the existing `brandingPresets`/`toggleBranding('watermark')`), Save/Share Now (both export-then-share — no separate download-to-disk API exists, so rather than have one silently do nothing they currently behave the same, noted in a code comment). Deleted `social_screen.dart`, `export_share_screen.dart`, `models/export_job.dart`. `app_state.dart`: removed `exportJobs`/`isBatchExporting`/`exportBatch`/`retryClipExport`; added `exportSelectedClip()`/`isExportingClip`/`exportError`/`exportedDownloadPath`; also fixed `_renderClip`'s logo wiring, which had a stale "not wired until Phase 6" comment — Phase 6 already built logo upload, so it now actually fetches and base64-encodes the logo when the watermark preset is on. `project_store.dart`: `ProjectSnapshot.exportJobs` field removed. `main.dart`: `/social`+`/export` routes replaced by `/share`. `video_editor_screen.dart`'s EXPORT button now points at `/share`.

### Phase 13 — Reskin + extend: Settings — DONE
Deleted `branding_screen.dart`, consolidated into a fully rebuilt `settings_screen.dart` matching `ui_kit/settings/index.html`: Church Profile (logo + name), Contact & Service Times (address + a dynamic list of label/time rows, not just the two the mockup shows — "Add Another Service Time" needed somewhere to add to), Default Caption Template (3 swatches mapped onto the existing `CaptionTemplate` enum: Bold Word→boldPop, Clean Line→minimal, Highlight→karaoke), Default Hashtags (removable chips + add), Giving Information (toggle, off by default, dimmed field + warning while off). `brand_settings.dart` extended with `address`/`serviceTimes` (new `ServiceTime` class)/`defaultCaptionTemplate`/`defaultHashtags`/`givingEnabled`/`givingAccountText` + a `copyWith`. `BrandSettings.empty` renamed to `BrandSettings.seeded` and given real default values (Higherlife Commission, address, Sunday/Thursday times, default hashtags, FNB account text) from CLAUDE.md's default church config — matches what the mockup itself shows as pre-filled content, not a blank slate. `AppRoutes.branding`/`/branding` route removed.

### Phase 14 — Cleanup pass — DONE
`test/widget_test.dart` already matched current reality (verified, no changes needed). Rewrote `README.md` from scratch — it was still describing the pre-MVP1 mock-data prototype (wrong screens, wrong theme file, no backend at all). Removed genuinely dead code found by grepping for zero callers: `AppState.detectSilence`/`applySilenceRemoval`/`transcribeAudio` (the Phase 5→9 transitional aliases, now that Phase 9's deletions left them uncalled) and `toggleSilenceRangeAccepted`/`setAllSilenceRangesAccepted`/`toggleClipIncludeInExport` (handlers for toggle UIs that no longer exist, Phases 9/11) — kept the underlying data fields (`silenceRanges`, `AiClip.includeInExport`), just removed the dead mutators. Fixed stale doc comments (`caption_style.dart` referencing the deleted Captions screen and the deleted `FfmpegService` class). `pubspec.yaml` description updated (still said "batch-export", Phase 12 dropped that). **Real bug found and fixed**, not just docs: Settings' "Default Caption Template" picker wrote to `BrandSettings.defaultCaptionTemplate`, but `_renderClip` (the actual export path) reads `AppState.captionStyle`, which nothing synced from it — the picker silently did nothing to real exports. `reloadBrandSettings()` now syncs `captionStyle`'s template from `brandSettings.defaultCaptionTemplate`. Left `android/` in place (untouched, unused by the web build) — removing native platform scaffolding is a bigger decision than a cleanup pass should make unilaterally.
